...
function Navbar() {
        ...
        async (pos) => {
          try {
            ...
            const city = data.address?.city || data.address?.town ||
              data.address?.village || data.address?.county || 'India';
            setLocation(city);
          } catch { setLocation('India'); }
        },
        () => setLocation('India')
      );
    } else { setLocation('India'); }
  }, []);
  ...
  useEffect(() => {
    ...
    const fetchNotifications = async () => {
      ...
      const notifs = [];
      try {
        const { data: cartItems } = await axios.get(`${API}/api/cart`, { headers });
        if (cartItems.length > 0) {
          notifs.push({ id: 'cart', text: `You have ${cartItems.length} item${cartItems.length > 1 ? 's' : ''} waiting in your cart`, time: 'Now', unread: true, icon: '🛒', link: '/cart' });
        }
      } catch {}
      try {
        const { data: orders } = await axios.get(`${API}/api/orders/my`, { headers });
        orders.slice(0, 3).forEach(order => {
          if (order.status === 'processing') notifs.push({ id: `order-${order.id}`, text: `Order #${order.id} is being processed`, time: new Date(order.created_at).toLocaleDateString(), unread: true, icon: '⚙️', link: '/orders' });
          if (order.status === 'shipped') notifs.push({ id: `order-shipped-${order.id}`, text: `Order #${order.id} has been shipped!`, time: new Date(order.created_at).toLocaleDateString(), unread: true, icon: '🚚', link: '/orders' });
          if (order.status === 'delivered') notifs.push({ id: `order-delivered-${order.id}`, text: `Order #${order.id} delivered! Leave a review`, time: new Date(order.created_at).toLocaleDateString(), unread: false, icon: '✅', link: '/orders' });
        });
      } catch {}
      try {
        const { data: posts } = await axios.get(`${API}/api/forum`);
        if (posts.length > 0) notifs.push({ id: 'forum', text: `${posts.length} post${posts.length > 1 ? 's' : ''} in the community forum`, time: 'Community', unread: false, icon: '💬', link: '/forum' });
      } catch {}
      if (notifs.length === 0) notifs.push({ id: 'welcome', text: 'Welcome to SafeHer! Start exploring products', time: 'Now', unread: false, icon: '🌸', link: '/' });
    ...
    <nav style={{ background: '#fff', boxShadow: '0 1px 0 #ede8ff',
      ...
      <div style={{ display: 'flex', alignItems: 'center', height: 64,
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6,
          ...
          onClick={() => { const loc = prompt('Enter your city:', location); if (loc) setLocation(loc); }}>
          <span style={{ fontSize: 13 }}>📍</span>
          <span style={{ fontSize: 12, color: '#7F77DD', fontWeight: 500 }}>{location}</span>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            ...
            <Link to="/cart" style={{ position: 'relative', display: 'flex',
              ...
              {cartCount > 0 && (
                <span style={{ position: 'absolute', top: -6, right: -6,
                  ...
                  alignItems: 'center', justifyContent: 'center' }}>{cartCount}</span>
              )}
            </Link>
          )}
            ...
            <div style={{ position: 'relative' }} ref={notifRef}>
              ...
              {showNotifs && (
                <div style={{ position: 'absolute', top: 48, right: 0, width: 300,
                  ...
                  <div style={{ padding: '14px 16px', borderBottom: '1px solid #f0eeff',
                    fontWeight: 600, fontSize: 14 }}>Notifications</div>
                  {notifications.length === 0 ? (
                    <div style={{ padding: '24px 16px', textAlign: 'center',
                      color: '#bbb', fontSize: 13 }}>No notifications yet</div>
                  ) : notifications.map(n => (
                    <div key={n.id}
          ...
          {user ? (
                ...
                <div style={{ position: 'absolute', top: 48, right: 0, width: 220,
                  ...
                  {[
                    ...
                    ...(user.role === 'seller' ? [
                      ...
                      { icon: '📋', label: 'Customer Orders', to: '/seller/orders' },
                    ] : []),
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <Link to="/login" style={{ fontSize: 13, fontWeight: 500, color: '#555',
      ...
      <div style={{ borderTop: '1px solid #f0eeff', background: '#fff',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4,
          padding: '0 16px', height: 44, width: 'max-content', minWidth: '100%' }}>
          {navLinks.map(link => (
...
function ComingSoon({ title, icon, desc }) {
  return (
    <div style={{ maxWidth: 500, margin: '80px auto', textAlign: 'center', padding: '0 24px' }}>
      ...
      <p style={{ color: '#888', fontSize: 16, lineHeight: 1.7, marginBottom: 28 }}>{desc}</p>
